"""
=============================================================================
SCRIPT NAME: Step Five Tcost.py - High-Performance Portfolio Optimization
=============================================================================
...
- T2_Trading_Cost.xlsx: Monthly one-way trading cost for each factor

OUTPUT FILES:
- T2_rolling_window_weights.xlsx: Optimized factor weights (hybrid window strategy)
- T2_strategy_statistics.xlsx: Strategy statistics with gross returns, transaction
  costs, net returns, factor turnover, and country turnover
...
VERSION: 2.1 - High Performance CVXPY Implementation with Transaction Costs
LAST UPDATED: 2026-03-15
AUTHOR: Claude Code

DESCRIPTION:
High-performance portfolio optimization using CVXPY with OSQP solver for fast
convex optimization. Implements hybrid window strategy:
- First 60 months: Expanding window (all available data)
- After 60 months: Rolling window (exactly 60 months)

The optimizer supports a linear rebalancing cost penalty using monthly
factor-level trading costs. The objective is:
- Maximize: w'μ - λ*w'Σw - γ*||w||² - TCost*Σ(cost_i * |Δw_i|)

This makes the optimizer prefer high-conviction trades and avoid unnecessary
turnover in factors with higher implementation costs.
...
5. Linear transaction cost integration per rebalance month
...
NOTES:
- `TCost` controls how strongly the optimizer penalizes trading activity
- `TCost = 0` disables transaction cost awareness
- Country trading costs are loaded from `Step Tcost.xlsx` column C
- Reported strategy returns are saved as both gross and net of transaction costs
- Output reports both factor-level and country-level turnover
=============================================================================
"""
...
TCost = 0.0               # Transaction cost multiplier applied to monthly linear trading costs
...
class FastPortfolioOptimizer:
    ...
    Maximize: w'μ - λ*w'Σw - γ*||w||² - tc*Σ(cost_i * |Δw_i|)
    Subject to: sum(w) = 1, 0 ≤ w ≤ max_weights
    """
    
    def __init__(self, n_assets, factor_names, lambda_param=1.0, hhi_penalty=0.01, tcost_multiplier=0.0, max_weights=None):
        """
        ...
        Args:
            ...
            tcost_multiplier: Linear transaction cost multiplier
        ...
        self.prev_country_weights = None
        
    def optimize_weights(self, expected_returns, covariance_matrix, country_exposure_matrix=None, country_trading_costs=None):
        """
        ...
        Args:
            ...
            country_exposure_matrix: Countries x factors mapping matrix for implied country weights
            country_trading_costs: np.array of per-country one-way trading costs
        ...
        if country_exposure_matrix is None:
            implied_country_weights = None
            country_turnover_penalty = 0
        else:
            country_exposure_matrix = np.nan_to_num(np.asarray(country_exposure_matrix, dtype=float), nan=0.0)
            implied_country_weights = country_exposure_matrix @ self.weights_var
            if country_trading_costs is None:
                country_trading_costs = np.ones(country_exposure_matrix.shape[0])
            else:
                country_trading_costs = np.nan_to_num(np.asarray(country_trading_costs, dtype=float), nan=0.0)
            if self.prev_country_weights is None:
                reference_country_weights = np.zeros(country_exposure_matrix.shape[0])
            else:
                reference_country_weights = self.prev_country_weights
            country_turnover_penalty = self.tcost_multiplier * 0.5 * cp.sum(
                cp.multiply(country_trading_costs, cp.abs(implied_country_weights - reference_country_weights))
            )
        
        # Objective: maximize utility (minimize negative utility)
        objective = cp.Maximize(portfolio_return - risk_penalty - concentration_penalty - country_turnover_penalty)
        ...
        try:
            ...
            if problem.status == cp.OPTIMAL:
                ...
                if country_exposure_matrix is not None:
                    self.prev_country_weights = np.asarray(country_exposure_matrix @ optimal_weights, dtype=float)
...
def load_and_prepare_data():
    ...
    Returns:
        tuple: (returns_df, max_weights_dict, trading_costs_df, country_exposure_matrices, country_names, country_trading_costs)
    """
    ...
    # Load monthly factor trading costs
    trading_costs = pd.read_excel('T2_Trading_Cost.xlsx', index_col=0)
    trading_costs.index = pd.to_datetime(trading_costs.index)
    ...
    trading_costs = trading_costs.reindex(columns=returns.columns)
    trading_costs = trading_costs.reindex(returns.index)

    if trading_costs.abs().mean().mean() > 1:
        trading_costs = trading_costs / 10000
    else:
        trading_costs = trading_costs / 100

    trading_costs = trading_costs.astype(float)
    
    country_exposure_matrices, country_names = build_country_exposure_matrices(
        factor_names=list(returns.columns),
        dates=returns.index
    )
    country_trading_costs = load_country_trading_costs(country_names)
    ...
    logging.info(f"Loaded trading cost data: {trading_costs.shape[0]} periods, {trading_costs.shape[1]} factors")
    logging.info(f"Built country exposure matrices for {len(country_exposure_matrices)} dates and {len(country_names)} countries")
    logging.info(f"Loaded country trading costs for {len(country_trading_costs)} countries")
    
    return returns, max_weights, trading_costs, country_exposure_matrices, country_names, country_trading_costs

def load_country_trading_costs(country_names):
    costs_df = pd.read_excel('Step Tcost.xlsx', sheet_name=0)
    costs_df = costs_df.rename(columns={costs_df.columns[0]: 'Country', costs_df.columns[2]: 'Trading Cost'})
    ...
    costs_series = costs_series.fillna(costs_series.mean())
    return costs_series.to_numpy(dtype=float)

def build_country_exposure_matrices(factor_names, dates):
    factor_df = pd.read_csv('Normalized_T2_MasterCSV.csv', usecols=['date', 'country', 'variable', 'value'])
    factor_df['date'] = pd.to_datetime(factor_df['date'])
    factor_df = factor_df[factor_df['variable'].isin(factor_names)].copy()
    factor_df['value'] = pd.to_numeric(factor_df['value'], errors='coerce')
    factor_df = factor_df.dropna(subset=['value'])
    country_names = factor_df['country'].drop_duplicates().tolist()
    date_groups = {date: group for date, group in factor_df.groupby('date')}
    factor_columns = pd.Index(factor_names)
    matrices = {}
    last_valid_column = {}

    for date in sorted(dates):
        if date not in date_groups:
            empty_matrix = np.full((len(country_names), len(factor_names)), np.nan, dtype=float)
            for col_idx, factor_name in enumerate(factor_names):
                if factor_name in last_valid_column:
                    empty_matrix[:, col_idx] = last_valid_column[factor_name]
            matrices[date] = empty_matrix
            continue

        slice_df = date_groups[date]
        pivot = slice_df.pivot(index='country', columns='variable', values='value')
        pivot = pivot.reindex(index=country_names, columns=factor_columns)

        rank_desc = pivot.rank(axis=0, method='first', ascending=False)
        counts = pivot.notna().sum(axis=0).replace(0, np.nan)
        counts_mat = np.tile(counts.values, (len(pivot.index), 1))
        rank_pct = rank_desc.values / counts_mat
        ...
        fuzzy = np.nan_to_num(full_mask + taper, nan=0.0)

        col_sums = fuzzy.sum(axis=0)
        col_sums[col_sums == 0] = 1.0
        matrix = fuzzy / col_sums

        for col_idx, factor_name in enumerate(factor_names):
            if np.isfinite(matrix[:, col_idx]).any() and matrix[:, col_idx].sum() > 0:
                last_valid_column[factor_name] = matrix[:, col_idx].copy()
            elif factor_name in last_valid_column:
                matrix[:, col_idx] = last_valid_column[factor_name]
            else:
                matrix[:, col_idx] = np.nan

        matrices[date] = matrix

    next_valid_column = {}
    for date in reversed(sorted(dates)):
        matrix = matrices[date]
        for col_idx in range(len(factor_names)):
            column = matrix[:, col_idx]
            if np.isfinite(column).any():
                next_valid_column[factor_names[col_idx]] = column.copy()
            elif factor_names[col_idx] in next_valid_column:
                matrix[:, col_idx] = next_valid_column[factor_names[col_idx]]
        matrices[date] = matrix

    uniform_column = np.full(len(country_names), 1.0 / len(country_names), dtype=float)
    for date in dates:
        matrix = matrices[date]
        for col_idx in range(len(factor_names)):
            column = matrix[:, col_idx]
            if not np.isfinite(column).any():
                matrix[:, col_idx] = uniform_column
        matrices[date] = np.nan_to_num(matrix, nan=0.0)

    return matrices, country_names
...
def run_fast_optimization():
    ...
    # Load data
    returns_df, max_weights, trading_costs_df, country_exposure_matrices, country_names, country_trading_costs = load_and_prepare_data()
    ...
    country_weights_df = pd.DataFrame(0.0, index=extended_dates, columns=country_names)
    ...
    for i, date in enumerate(optimization_dates):
        ...
        country_exposure_matrix = country_exposure_matrices.get(date)
        ...
        if country_exposure_matrix is not None:
            country_weights_df.loc[date] = country_exposure_matrix @ optimal_weights
    ...
    if country_exposure_matrices:
        latest_exposure_date = max(country_exposure_matrices.keys())
        extra_country_exposure_matrix = country_exposure_matrices[latest_exposure_date]
    else:
        extra_country_exposure_matrix = np.zeros((len(country_names), len(factor_names)), dtype=float)
    optimal_weights_extra = optimizer.optimize_weights(
        expected_returns_extra,
        cov_matrix_extra,
        extra_country_exposure_matrix,
        country_trading_costs
    )
    ...
    country_weights_df.loc[next_month_date] = extra_country_exposure_matrix @ optimal_weights_extra
    ...
    return weights_df, country_weights_df, returns_df, trading_costs_df, country_trading_costs

def calculate_strategy_performance(weights_df, country_weights_df, returns_df, trading_costs_df, country_trading_costs):
    """
    ...
    aligned_dates = []
    previous_country_weights = np.zeros(country_weights_df.shape[1], dtype=float)
    ...
    for date in weights_df.index:
        if date in returns_df.index:
            ...
            if next_month_idx < len(returns_df.index):
                ...
                country_weights = country_weights_df.loc[date].to_numpy(dtype=float)
                returns = returns_df.loc[next_month_date].values
                
                # Calculate gross and net portfolio return
                gross_portfolio_return = np.sum(weights * returns)
                realized_transaction_cost = TCost * 0.5 * np.sum(
                    country_trading_costs * np.abs(country_weights - previous_country_weights)
                )
                portfolio_return = gross_portfolio_return - realized_transaction_cost
                gross_portfolio_returns.append(gross_portfolio_return)
                portfolio_returns.append(portfolio_return)
                transaction_cost_series.append(realized_transaction_cost)
                aligned_dates.append(next_month_date)
                previous_country_weights = country_weights.astype(float)
    
    # Create portfolio returns series
    portfolio_returns_series = pd.Series(portfolio_returns, index=aligned_dates)
    gross_portfolio_returns_series = pd.Series(gross_portfolio_returns, index=aligned_dates)
    transaction_costs_series = pd.Series(transaction_cost_series, index=aligned_dates)
    ...
    # Turnover analysis
    factor_weight_changes = weights_df.diff().abs().fillna(0)
    monthly_factor_turnover = factor_weight_changes.sum(axis=1) / 2
    avg_factor_turnover = monthly_factor_turnover.mean()

    country_weight_changes = country_weights_df.diff().abs().fillna(0)
    monthly_country_turnover = country_weight_changes.sum(axis=1) / 2
    avg_country_turnover = monthly_country_turnover.mean()

    factor_names = list(weights_df.columns)
    exposure_dates = [date for date in weights_df.index if date in returns_df.index]
    country_exposure_matrices, _ = build_country_exposure_matrices(factor_names, exposure_dates)

    mapping_drift_values = []
    country_turnover_ex_mapping_values = []
    previous_date = None

    for date in weights_df.index:
        if previous_date is None or date not in country_exposure_matrices or previous_date not in country_exposure_matrices:
            mapping_drift_values.append(0.0)
            country_turnover_ex_mapping_values.append(0.0)
        else:
            previous_factor_weights = weights_df.loc[previous_date].to_numpy(dtype=float)
            current_factor_weights = weights_df.loc[date].to_numpy(dtype=float)
            previous_matrix = country_exposure_matrices[previous_date]
            current_matrix = country_exposure_matrices[date]

            mapping_drift = 0.5 * np.sum(
                np.abs((current_matrix @ previous_factor_weights) - (previous_matrix @ previous_factor_weights))
            )
            country_turnover_ex_mapping = 0.5 * np.sum(
                np.abs((current_matrix @ current_factor_weights) - (current_matrix @ previous_factor_weights))
            )

            mapping_drift_values.append(mapping_drift)
            country_turnover_ex_mapping_values.append(country_turnover_ex_mapping)

        previous_date = date

    monthly_mapping_drift = pd.Series(mapping_drift_values, index=weights_df.index)
    monthly_country_turnover_ex_mapping = pd.Series(country_turnover_ex_mapping_values, index=weights_df.index)
    avg_mapping_drift = monthly_mapping_drift.mean()
    avg_country_turnover_ex_mapping = monthly_country_turnover_ex_mapping.mean()
    ...
    stats = {
        ...
        'Average Monthly Factor Turnover (%)': avg_factor_turnover * 100,
        'Average Monthly Country Turnover (%)': avg_country_turnover * 100,
        'Average Monthly Country Turnover ex Mapping Drift (%)': avg_country_turnover_ex_mapping * 100,
        'Average Monthly Mapping Drift (%)': avg_mapping_drift * 100,
        'Average Monthly Transaction Cost (%)': transaction_costs_series.mean() * 100,
    ...
    results = {
        ...
        'gross_monthly_returns': gross_portfolio_returns_series,
        'monthly_transaction_costs': transaction_costs_series,
        'monthly_factor_turnover': monthly_factor_turnover,
        'monthly_country_turnover': monthly_country_turnover,
        'monthly_country_turnover_ex_mapping': monthly_country_turnover_ex_mapping,
        'monthly_mapping_drift': monthly_mapping_drift
    }
...
def save_results(weights_df, performance_results):
    ...
    with pd.ExcelWriter(stats_output_file, engine='xlsxwriter') as writer:
        ...
        returns_df = pd.DataFrame({
            'Hybrid Strategy Gross Returns': performance_results['gross_monthly_returns'],
            'Transaction Costs': performance_results['monthly_transaction_costs'],
            'Hybrid Strategy Net Returns': performance_results['monthly_returns']
        })
        returns_df.to_excel(writer, sheet_name='Monthly Returns')
        
        # Monthly turnover
        turnover_df = pd.DataFrame({
            'Monthly Factor Turnover': performance_results['monthly_factor_turnover'],
            'Monthly Country Turnover': performance_results['monthly_country_turnover'],
            'Monthly Country Turnover ex Mapping Drift': performance_results['monthly_country_turnover_ex_mapping'],
            'Monthly Mapping Drift': performance_results['monthly_mapping_drift']
        })
...
def main():
    ...
    try:
        # Run optimization
        weights_df, country_weights_df, returns_df, trading_costs_df, country_trading_costs = run_fast_optimization()
        
        # Calculate performance
        performance_results = calculate_strategy_performance(
            weights_df,
            country_weights_df,
            returns_df,
            trading_costs_df,
