"""
=============================================================================
SCRIPT NAME: Step Five FAST Slope.py - Slope-Based Factor Timing Optimization
=============================================================================
...
OUTPUT FILES:
- T2_rolling_window_weights.xlsx: Optimized factor weights (slope-adjusted hybrid strategy)
...
VERSION: 3.0 - Slope-Based Factor Timing with CVXPY Implementation
LAST UPDATED: 2025-06-17
AUTHOR: Claude Code

DESCRIPTION:
Advanced portfolio optimization implementing slope-based factor timing methodology.
Uses 24-month rolling regression slopes on cumulative factor returns to dynamically
adjust expected returns, amplifying exposure to factors with positive momentum
and reducing exposure to factors with negative momentum.

Mathematical Foundation:
- Slope = β = Σ((x_i - x̄)(y_i - ȳ)) / Σ((x_i - x̄)²)
- Adjusted Expected Return = Base Expected Return + (10 × Slope)
- Positive slope: BUY signal (increases allocation)
- Negative slope: SELL/REDUCE signal (decreases allocation)

Optimization Approach:
1. Calculate cumulative geometric returns: (1 + monthly_returns).cumprod() - 1
2. Compute 24-month rolling regression slopes using scikit-learn LinearRegression
3. Apply slope adjustments: expected_returns = base_returns + 10 × slope
4. Optimize using CVXPY with OSQP solver for 50-100x speed improvement
5. Hybrid window strategy:
   - First 60 months: Expanding window (all available data)
   - After 60 months: Rolling window (exactly 60 months)

Key Features:
1. Real-time slope calculation using latest available data
2. Robust error handling with fallback to base expected returns
3. Minimum 24-month requirement for statistical validity
4. Dynamic factor timing based on performance momentum
5. CVXPY/OSQP convex optimization with warm-start capabilities
...
- matplotlib
- openpyxl
- scikit-learn (for slope calculations)

USAGE:
python "Step Five FAST Slope.py"

NOTES:
- Implements slope-based factor timing for enhanced factor selection
...
- Graceful fallback to base optimization if slope calculation fails
=============================================================================
"""
...
def calculate_cumulative_returns(monthly_returns):
    ...
    Args:
        monthly_returns: DataFrame of monthly factor returns

    Returns:
        DataFrame: Cumulative geometric returns
    """
...
def calculate_24m_slopes(cumulative_data):
    ...
    Args:
        cumulative_data: DataFrame of cumulative factor returns
    ...
    for factor in cumulative_data.columns:
        ...
        try:
            ...
            logging.warning(f"Could not calculate slope for {factor}: {e}")
            slopes[factor] = 0.0

    return slopes

def apply_slope_adjustments(expected_returns_base, cumulative_returns):
    """
    Apply slope-based adjustments to expected returns.

    Args:
        expected_returns_base: Series of base expected returns
        cumulative_returns: DataFrame of cumulative returns for slope calculation

    Returns:
        pd.Series: Slope-adjusted expected returns
    """
    try:
        # Calculate slopes
        slope_adjustments = calculate_24m_slopes(cumulative_returns)

        # Apply adjustments
        adjusted_returns = expected_returns_base.copy()
        for i, factor in enumerate(expected_returns_base.index):
            if factor in slope_adjustments:
                slope = slope_adjustments[factor]
                adjusted_returns.iloc[i] = adjusted_returns.iloc[i] + 10 * slope

        # Log slope adjustment statistics
        positive_slopes = sum(1 for s in slope_adjustments.values() if s > 0.001)
        negative_slopes = sum(1 for s in slope_adjustments.values() if s < -0.001)
        neutral_slopes = len(slope_adjustments) - positive_slopes - negative_slopes

        logging.info(f"Slope adjustments applied: {positive_slopes} positive, {negative_slopes} negative, {neutral_slopes} neutral")

        return adjusted_returns

    except Exception as e:
        logging.warning(f"Could not apply slope adjustments: {e}. Using base expected returns.")
        return expected_returns_base
...
def load_and_prepare_data():
    ...
    Returns:
        tuple: (returns_df, max_weights_dict, cumulative_returns_df)
    """
    ...
    # Calculate cumulative returns for slope analysis
    ...
    logging.info("Calculated cumulative returns for slope-based analysis")

    return returns, max_weights, cumulative_returns

def calculate_rolling_statistics(returns_df, cumulative_returns, window_size):
    """
    Pre-compute all rolling statistics for batch optimization with slope adjustments.

    Args:
        returns_df: DataFrame of factor returns
        cumulative_returns: DataFrame of cumulative returns for slope analysis
        window_size: Rolling window size (60 months)

    Returns:
        tuple: (expected_returns_dict, covariance_dict, dates_list)
    """
    logging.info("Pre-computing rolling statistics with slope adjustments...")
    ...
    for i, date in enumerate(dates[1:], 1):  # Start from month 2
        ...
        # Calculate base expected returns (matching original Step Five line 433)
        factor_means = window_data.mean(axis=0)
        expected_returns_base = 8 * factor_means

        # Apply slope adjustments using cumulative data up to current date
        cumulative_window = cumulative_returns.loc[:date]
        expected_returns = apply_slope_adjustments(expected_returns_base, cumulative_window)
    ...
    logging.info(f"Pre-computed statistics with slope adjustments for {len(dates_list)} optimization periods")
...
def run_fast_optimization():
    ...
    # Load data
    returns_df, max_weights, cumulative_returns = load_and_prepare_data()
    ...
    # Pre-compute all rolling statistics with slope adjustments
    expected_returns_dict, covariance_dict, optimization_dates = calculate_rolling_statistics(
        returns_df, cumulative_returns, WINDOW_SIZE
    )
    ...
    logging.info(f"Calculating extra month optimization with slope adjustments for {next_month_date.strftime('%Y-%m')}")
    ...
    # Calculate base expected returns for extra month
    factor_means = extra_month_data.mean(axis=0)
    expected_returns_base_extra = 8 * factor_means  # Apply 8x scaling factor

    # Apply slope adjustments for extra month using full cumulative data
    expected_returns_extra = apply_slope_adjustments(expected_returns_base_extra, cumulative_returns)
    ...
    # Optimize weights for extra month with slope-adjusted expected returns
