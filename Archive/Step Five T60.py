"""
=============================================================================
SCRIPT NAME: Step Five T60.py - High-Performance Portfolio Optimization
=============================================================================

INPUT FILES:
- T2_Optimizer.xlsx: Monthly factor returns from Step Four
- T60.xlsx (first worksheet): Trailing 60-month average factor returns (as %), same
  timing as shift(1).rolling(60) on Step Four net returns — verified to match
  the arithmetic mean window used historically for expected returns
...
VERSION: 2.1 - Expected returns from T60.xlsx
LAST UPDATED: 2026-03-19
AUTHOR: Claude Code

DESCRIPTION:
High-performance portfolio optimization using CVXPY with OSQP solver for 50-100x 
speed improvement over scipy.optimize. T60.xlsx is the driver: expected returns (mu)
come only from T60 (8 × T60/100 as decimal monthly means). Any calendar month with
no T60 row (or NaN in any factor) is skipped — no optimization and no fallback from
T2_Optimizer returns for mu. Covariance Σ still uses the hybrid return window on
T2_Optimizer for months that are optimized:
- First 60 months of that month's history: expanding window
- After that: rolling 60 months
...
T60_XLSX = "T60.xlsx"      # First sheet: trailing 60m avg returns (%), from Step Four
# EMA_DECAY removed - usin32g simple arithmetic mean like original Step Five
# =============================================================================
# SETUP LOGGING
# =============================================================================
...
def load_t60_forecasts(returns_df: pd.DataFrame) -> pd.DataFrame:
    """
    Load T60.xlsx (first worksheet; sheet name does not matter). Values are stored as
    percentages (×100 vs decimal returns).

    Columns must match factor columns in T2_Optimizer (after CS column drop).
    """
    if not os.path.exists(T60_XLSX):
        raise FileNotFoundError(
            f"T60 file not found: {T60_XLSX} (run Step Four to build trailing 60-month averages)"
        )
    t60 = pd.read_excel(T60_XLSX, sheet_name=0, index_col=0)
    t60.index = pd.to_datetime(t60.index)
    t60 = t60.sort_index()
    missing_cols = [c for c in returns_df.columns if c not in t60.columns]
    if missing_cols:
        raise ValueError(
            "T60.xlsx is missing factor columns present in T2_Optimizer.xlsx: "
            f"{missing_cols[:10]}{'...' if len(missing_cols) > 10 else ''}"
        )
    return t60[returns_df.columns].astype(float)


def calculate_rolling_statistics(returns_df, window_size, t60_df):
    """
    Pre-compute covariance windows and expected returns from T60.xlsx per date.

    Only dates with a full non-NaN T60 row are included; others are skipped entirely.

    Args:
        returns_df: DataFrame of factor returns (decimals)
        window_size: Rolling window size (60 months)
        t60_df: Aligned T60 forecasts (%); same row date as optimization date

    Returns:
        tuple: (expected_returns_dict, covariance_dict, dates_list)
    """
    logging.info("Pre-computing rolling statistics (covariance from returns, mu from T60.xlsx only)...")
    ...
    for i, date in enumerate(dates[1:], 1):  # Start from month 2
        ...
        # T60 drives inclusion: no row or any NaN → skip this month completely
        if date not in t60_df.index:
            skipped_no_t60 += 1
            continue
        t60_row = t60_df.loc[date]
        if t60_row.isna().any():
            skipped_no_t60 += 1
            continue

        # T60 is in percent; convert to decimal monthly mean, then same 8× scaling as before
        expected_returns = 8.0 * (t60_row.values / 100.0)
    ...
    logging.info(
        "Pre-computed statistics for %d optimization periods (%d candidate months skipped: no T60 row or NaN in T60)",
        len(dates_list),
        skipped_no_t60,
    )
    if not dates_list:
        raise RuntimeError(
            "No optimization dates: every month lacked a usable T60 row. "
            "Extend T60.xlsx (e.g. rerun Step Four) or fix NaNs."
        )
...
def run_fast_optimization():
    ...
    t60_df = load_t60_forecasts(returns_df)

    # Forward month (only optimized if T60 has that row)
    ...
    include_extra = next_month_date in t60_df.index and not t60_df.loc[next_month_date].isna().any()
    if include_extra:
        logging.info("Extra month %s: T60 present — will optimize forward weights.", next_month_date.strftime("%Y-%m"))
    else:
        logging.info(
            "Extra month %s: skipped (no T60 row or NaN). T60 is the driver.",
            next_month_date.strftime("%Y-%m"),
        )

    extended_dates = optimization_dates + ([next_month_date] if include_extra else [])
    ...
    if include_extra:
        logging.info("Calculating extra month optimization for %s", next_month_date.strftime("%Y-%m"))
        start_idx = max(0, len(returns_df.index) - WINDOW_SIZE)
        extra_month_data = returns_df.iloc[start_idx:]
        expected_returns_extra = 8.0 * (t60_df.loc[next_month_date].values / 100.0)
...
def main():
    ...
    logging.info("="*80)
    logging.info("T2 FACTOR TIMING - STEP FIVE T60: HIGH-PERFORMANCE OPTIMIZATION")
    logging.info("="*80)
    ...
    try:
        ...
        logging.info("="*80)
        logging.info("STEP FIVE T60 COMPLETED SUCCESSFULLY")
        logging.info("="*80)
        
    except Exception as e:
        logging.error(f"Error in Step Five T60: {str(e)}")
        import traceback
        traceback.print_exc()
        raise
