...
"""
=============================================================================
SCRIPT NAME: Step Three T2 GDELT Combined Top20 Portfolios Fast.py
=============================================================================
...
OUTPUT FILES:
- T2_GDELT_Combined_Top20.xlsx
- T2_GDELT_Combined_Top20.pdf
...
DESCRIPTION:
Same soft 15–25% portfolio logic as T2 / GDELT Step Three. Skips multi-month returns and
the same technical / price T2 exclusions as ``Step Three Top20 Portfolios Fast.py``; all
other columns (including ``GDELT_*`` factors) are analyzed.
...
# Match Step Three Top20 Portfolios Fast.py (T2) — GDELT names are prefixed so they are not skipped.
...
def run_portfolio_analysis(data_path: str, benchmark_path: str, output_dir: str) -> None:
    print("\nStarting T2+GDELT combined portfolio analysis…")
    ...
    excel_path = os.path.join(output_dir, "T2_GDELT_Combined_Top20.xlsx")
    results.to_excel(excel_path, index=False, float_format="%.2f")

    pdf_path = os.path.join(output_dir, "T2_GDELT_Combined_Top20.pdf")
    ...
    exposure_path = os.path.join(output_dir, "T2_GDELT_Combined_Top_20_Exposure.csv")
