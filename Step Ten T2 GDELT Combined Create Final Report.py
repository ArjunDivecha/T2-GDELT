"""
T2 + GDELT Combined Strategy Report — Step Ten
=============================================

INPUT FILES (combined track):
- T2_GDELT_Combined_Top20.xlsx, T2_GDELT_Combined_Optimizer.xlsx
...
OUTPUT:
- T2_GDELT_Combined_Strategy_Report_Comprehensive_YYYY-MM-DD.pdf
...
Version: 2.1 T2+GDELT Combined track
Date: 2026-04-02
"""
...
# Define input files
top20_file = 'T2_GDELT_Combined_Top20.xlsx'
optimizer_file = 'T2_GDELT_Combined_Optimizer.xlsx'
factor_weights_file = 'T2_GDELT_Combined_rolling_window_weights.xlsx'
country_weights_file = 'T2_GDELT_Combined_Final_Country_Weights.xlsx'
portfolio_returns_file = 'T2_GDELT_Combined_Final_Portfolio_Returns.xlsx'
strategy_stats_file = 'T2_GDELT_Combined_strategy_statistics.xlsx'

# Output report file
output_file = f'T2_GDELT_Combined_Strategy_Report_Comprehensive_{report_date}.pdf'

class PDF(FPDF):
    def header(self):
        # Title
        self.set_font('Arial', 'B', 18)  
        self.cell(0, 10, 'T2 + GDELT Combined Strategy Report - Comprehensive', 0, 1, 'C')
...
def create_comprehensive_report():
    """
    Generate the comprehensive T2+GDELT combined strategy report with all sections from both versions.
    """
    print("Generating Comprehensive T2 + GDELT Combined Strategy Report...")
    ...
    pdf.cell(0, 20, 'T2 + GDELT Combined Strategy Report', 0, 1, 'C')
    ...
    pdf.set_font('Arial', '', 12)
    pdf.multi_cell(0, 5, 'The following table shows the top 10 portfolios from our T2+GDELT Combined Top20 analysis.')
    ...
    pdf.set_font('Arial', '', 12)
    pdf.multi_cell(0, 5, 'Analysis of the latest factor weights for the T2+GDELT combined strategy.')
    ...
    try:
        ...
        if len(sample_weights) == 0:
            ...
            pdf.ln(5)
            pdf.set_font('Arial', 'I', 9)
            pdf.multi_cell(0, 5, 'Source: T2+GDELT Combined Factor Weight Analysis - Full Period')
            ...
            if len(recent_sample_weights) > 0:
                ...
                pdf.ln(5)
                pdf.set_font('Arial', 'I', 9)
                pdf.multi_cell(0, 5, 'Source: T2+GDELT Combined Factor Weight Analysis - Recent 3 Years')
            else:
                pdf.set_font('Arial', '', 11)
    ...
    pdf.set_font('Arial', '', 12)
    pdf.multi_cell(0, 5, 'Current country weights for the T2+GDELT combined strategy portfolio.')
    ...
    summary_text = f"""
This comprehensive T2+GDELT Combined Strategy Report contains detailed analysis across 8 key sections:
...
This report provides comprehensive insights into the T2+GDELT combined strategy performance,
...
def generate_report_with_options():
    ...
    """
    print("=" * 60)
    print("T2+GDELT Combined Strategy Report Generator - Comprehensive Version 2.1")
    print("=" * 60)
