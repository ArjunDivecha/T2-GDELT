"""
=============================================================================
SCRIPT NAME: Step Five GDELT FAST.py - GDELT factor optimization
=============================================================================

INPUT FILES:
- GDELT_Optimizer.xlsx: Monthly factor returns from Step Four GDELT
- GDELT_RSQ.xlsx (sheet Monthly_RSQ): Per-factor R² on 12-month cumulative net returns
  (Step Four). When USE_RSQ_MODIFIER is True, each month’s expected return vector is
  multiplied factor-by-factor by an RSQ-derived multiplier; strength is set by RSQ_INFLUENCE
  (0 = neutral only, 1 = full RSQ effect; missing RSQ → RSQ_MISSING_MULTIPLIER).
- Step Factor Categories GDELT.xlsx: Factor categories and maximum weight constraints

OUTPUT FILES:
- GDELT_rolling_window_weights.xlsx
- GDELT_strategy_statistics.xlsx
- GDELT_factor_weight_heatmap.pdf
- GDELT_strategy_performance.pdf

VERSION: 1.3 - Parallel to Step Five FAST (GDELT factors)
...
NOTES:
- Rows in GDELT_Optimizer.xlsx that are all-NaN (e.g. stub future month) are dropped
  before optimization so trailing-window covariance stays finite for CVXPY quad_form.
- With USE_RSQ_MODIFIER True, μ used in the objective is (8 × window mean return) × m, where
  m = RSQ_MISSING_MULTIPLIER + RSQ_INFLUENCE × (RSQ − RSQ_MISSING_MULTIPLIER) per factor/month
  (then floored at 0). RSQ_INFLUENCE=0 disables RSQ shaping (m constant); RSQ_INFLUENCE=1 is full RSQ.
  Covariance is unchanged. Set USE_RSQ_MODIFIER False to revert.
...
def load_and_prepare_data():
    ...
    # Load returns data
    returns = pd.read_excel('GDELT_Optimizer.xlsx', index_col=0)
    ...
    # Step Four always writes monthly net returns as percentage points (decimal × 100).
    # Do not use mean>1 heuristic — small average |return| leaves values undivided and
    # inflates strategy statistics (fake 40%+ annual returns).
    ...
    # Drop months where every factor is NaN (e.g. placeholder / incomplete last row in
    # GDELT_Optimizer.xlsx). Including such a row in the trailing WINDOW_SIZE window
    # makes np.cov all-NaN and CVXPY quad_form fails ("must be symmetric/Hermitian").
    ...
    if n_f > n_t:
        logging.warning(
            ...
            "stats may be meaningless.",
            n_f,
            n_t,
        )
    if n_f > WINDOW_SIZE:
        logging.warning(
            "Factor count (%d) > WINDOW_SIZE (%d): severe overfitting risk on rolling covariance.",
            n_f,
            WINDOW_SIZE,
        )
...
def load_rsq_modifier_frame(returns_df: pd.DataFrame) -> pd.DataFrame:
    """
    Load GDELT_RSQ.xlsx and align index/columns to returns_df (month stamps, factor names).
    """
    ...
    if not os.path.isfile(RSQ_PATH):
        raise FileNotFoundError(
            ...
            "Run Step Four GDELT or set USE_RSQ_MODIFIER = False."
        )
...
def rsq_multiplier_vector(
    ...
    """
    One row of multipliers aligned to factor_names. Uses ``date`` if present in rsq_df;
    else ``fallback_date``; else all RSQ_MISSING_MULTIPLIER.
    """
    if rsq_df.empty:
...
def apply_rsq_to_expected_returns(
    ...
    """Element-wise multiply forecast by RSQ multipliers (Series index aligned)."""
    s = (
        expected_returns
...
def create_factor_weight_heatmap(weights_df, top_n=20):
    ...
    plt.savefig('GDELT_factor_weight_heatmap.pdf',
...
def create_strategy_performance_plot(performance_results):
    ...
    plt.title('GDELT Strategy Cumulative Performance', fontsize=16, fontweight='bold', pad=20)
    ...
    plt.tight_layout()
    plt.savefig('GDELT_strategy_performance.pdf', dpi=300, bbox_inches='tight')
    plt.close()
    
    logging.info("Strategy performance plot saved to GDELT_strategy_performance.pdf")
...
def main():
    ...
    logging.info("="*80)
    logging.info("GDELT TRACK - STEP FIVE GDELT FAST: OPTIMIZATION")
    logging.info("="*80)
    ...
    try:
        ...
        logging.info("="*80)
        logging.info("STEP FIVE GDELT FAST COMPLETED SUCCESSFULLY")
        logging.info("="*80)
        
    except Exception as e:
        logging.error(f"Error in Step Five GDELT FAST: {str(e)}")
        import traceback
        traceback.print_exc()
        raise
